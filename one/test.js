...
function testing123(){
  ...
  for (let i = 3; i < uniqueIntegers.length; i = i + 2) {
    ...
    const r2 = expect(
      avlTree.search(uniqueIntegers[i]).getKey(),
      uniqueIntegers[i],
      `key ${
        uniqueIntegers[i]
      } in tree`,
    ...
    if(n) {const nl = n.getLeft()
    const nr = n.getRight()
    const nlh = nl ? nl.getHeight() : 0
    const nrh = nr ? nr.getHeight() : 0
    const bf = nlh - nrh
    const r3 = expect(
      [1, -1, 0].includes(bf),
      true,
  ...
  const bulkAVL = avl(Array.from({ length: 1000 }, (_, index) => index));
  let r = expect(
    bulkAVL.length(),
    1000,
  ...
  // union
  const ua = avl([1,2,3,4])
  const ub = avl([2,3,4,5])
  ...
  if(!ru6) falses++
  // intersect
  const ia = avl([1,2,3,4])
  const ib = avl([2,3,4,5])
  ...
  if(!ri4) falses++
  // difference
  const da = avl([1,2,3,4])
  const db = avl([2,3,4,5])
